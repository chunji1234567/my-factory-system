...
- `order_no`：缺省时由 `SalesOrderSerializer._generate_order_no` 生成，**2026-06-19 起格式 `SO-yyyymmdd-NNNN`**（日期段保留可读性，4 位序号**年内全局连续**、跨年重置 0001）。例：`SO-20260619-0042` → `SO-20260620-0043`（新一天不归零）→ `SO-20270101-0001`（跨年）。旧版本曾是 `SO-yyyymmdd-NNN` 按日重置 3 位（2026-06-18~06-19）和更早 `SO{year}-{NNNN}`；三种 prefix 不前缀相交。与采购单共用 helper `_generate_sequential_order_no(model, scan_prefix='SO-2026', full_prefix='SO-20260619', width=4)`：事务内 `select_for_update` 锁年内所有订单 + Python 里按 int 取最大尾号 +1（不依赖 SQL 字符串排序，防止旧 3 位 + 新 4 位混宽时 '100' > '0099' 的错位）
...
- `order_no` 缺省格式：**2026-06-19 起 `PO-yyyymmdd-NNNN`**——日期段保留可读性、4 位序号**年内全局连续**、跨年重置（与销售单同口径，详见 SalesOrder 段说明）。旧版本曾是 `PO-yyyymmdd-NNN` 按日重置（2026-06-18~06-19）和更早 `PO{year}-{NNNN}`
