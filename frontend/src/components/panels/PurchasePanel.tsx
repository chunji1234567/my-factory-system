              ...
              <div className="mt-3 space-y-2 text-xs text-slate-500">
                {order.items.slice(0, 2).map((item) => (
                  <p key={item.id} className="truncate">
                    {item.product_detail?.model_name || `物料#${item.product}`} · {Number(item.received_quantity)}/{Number(item.quantity)}
                  </p>
                ))}
                {order.items.length > 2 && <p>…共 {order.items.length} 条明细</p>}
              </div>
              <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-slate-500">
                <span>金额：¥ {Number(order.total_amount).toFixed(2)}</span>
